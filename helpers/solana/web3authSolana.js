...
const {
  ...
  SolanaWallet,
} = require("@web3auth/solana-provider");
...
const privateKey =
  ...
  fs.readFileSync(path.resolve(appRootPath.path, "privateKey.pem"));

const privateKeyProvider = new SolanaPrivateKeyProvider({
  config: {
    chainConfig: {
      chainNamespace: CHAIN_NAMESPACES.SOLANA_RPC_PROVIDER,
      chainId: "0x1",
      rpcTarget: process.env.SOLANA_RPC_PROVIDER,
 ...
 * @returns { Promise<SolanaWallet | Error> }
 */
const createPrivateKey = (chatId, walletNumber) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      //
      // const solanaPrivateKey = await web3authNodeprovider.request({
      ...
      resolve(solanaWallet);
    } catch (err) {
      // error logging
      logger.error("CREATE PRIVATE KEY WEB3AUTH SOLANA ERROR: " + err.message);
...
const getPrivateKey = (chatId, walletNumber) => {
  return new Promise(async (resolve, reject) => {
    try {
      const user = {
        id: chatId.toString() + walletNumber.toString(), // must be unique to each user
      ...
      // console.log({ solanaPrivateKey });
      resolve(solanaPrivateKey);
    } catch (err) {
      // error logging
      logger.error("GET PRIVATE KEY WEB3AUTH SOLANA ERROR: " + err.message);
