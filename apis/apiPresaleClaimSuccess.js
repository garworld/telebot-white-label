...
const apiPresaleClaimSuccess = async (request, reply) => {
  try {
    // // check env
    ...
    if (presale.length === 1) {
      ...
      if (presale.data[0].endtime > (new Date().getTime()) / 1000) {
        ...
        //
        if (spoofedTokenParsed.lt(softcappedParsed)) {
          //
          return reply.code(404).send({
            message: "Not Found, Softcap not Fulfilled",
          });
        }
      }

      //
      const receipt = await itoJeeted.claimSuccess();
      await receipt.wait();
    ...
    console.error("API PRESALE CLAIM SUCCESS ERROR: ", e);
    // logger.error("API PRESALE CLAIM SUCCESS ERROR: " + e.message);
...
module.exports = apiPresaleClaimSuccess;