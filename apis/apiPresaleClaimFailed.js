...
const apiPresaleClaimFailed = async (request, reply) => {
  try {
    // // check env
    ...
    if (presale.data.length === 1) {
      ...
      const softcappedParsed = (presale.data[0].base_token ? ethers.utils.parseUnits(presale.data[0].softcap.toString(), 6) : ethers.utils.parseUnits(presale.data[0].softcap.toString()));

      //
      if (ethers.BigNumber.from(spoofedToken.toString()).gte(softcappedParsed)) {
        //
        return reply.code(404).send({
          message: "Not Found, Softcap Fulfilled",
        });
      }

      //
      const receipt = await itoJeeted.claimFailed();
      await receipt.wait();
    ...
    console.error("API PRESALE CLAIM FAILED ERROR: ", e);
    // logger.error("API PRESALE CLAIM FAILED ERROR: " + e.message);
...
module.exports = apiPresaleClaimFailed;