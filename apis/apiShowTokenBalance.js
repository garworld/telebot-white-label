...
const apiShowTokenBalance = async function (request, reply) {
  const { chain_used, wallet_number, sort_by } = request.query;
  try {
    const chainIdx = {
      1: 0,
      42161: 1,
    ...
    if (sort_by) {
      if (sort_by === "price") {
        sortedTokenList = token_list.sort((a, b) => {
...
module.exports = apiShowTokenBalance;
