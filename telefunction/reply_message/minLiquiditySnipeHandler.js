...
const { SNIPE_SETTINGS, SNIPE_MIN_LIQUIDITY_OPTIONS_ID } = require("../../constants/sniping");
const { ethUsd } = require("../../helpers/tokenPrice");

module.exports = async ({ bot, redis, msg, chains }) => {
  //
  const snipeTokenOptions = await redis.GET(msg.chat.id + SNIPE_MIN_LIQUIDITY_OPTIONS_ID);
  const snipeTokenHandler = await redis.GET(msg.chat.id + "_snipeminliqmsg");

  snipeTokenOptions ? await redis.DEL(msg.chat.id + SNIPE_MIN_LIQUIDITY_OPTIONS_ID) : null;
  snipeTokenHandler ? await redis.DEL(msg.chat.id + "_snipeminliqmsg") : null;
  ...
  if (isNaN(Number(msg.text))) {
    ...
    if (theSnipingCache) {
      ...
      //
      sniperMode.min_liquidity = Number(msg.text);
