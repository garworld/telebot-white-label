...
const { SNIPE_SETTINGS, SNIPE_APPROVE_GWEI_OPTIONS_ID } = require("../../constants/sniping");
const { ethUsd } = require("../../helpers/tokenPrice");

module.exports = async ({ bot, redis, msg, chains }) => {
  //
  const snipeTokenOptions = await redis.GET(msg.chat.id + SNIPE_APPROVE_GWEI_OPTIONS_ID);
  const snipeTokenHandler = await redis.GET(msg.chat.id + "_snipeapprovegweimsg");

  snipeTokenOptions ? await redis.DEL(msg.chat.id + SNIPE_APPROVE_GWEI_OPTIONS_ID) : null;
  snipeTokenHandler ? await redis.DEL(msg.chat.id + "_snipeapprovegweimsg") : null;
  ...
  if (isNaN(Number(msg.text))) {
    ...
    bot.editMessageText("Wrong Approve Gwei Input", {
      parse_mode: "HTML",
    ...
    if (theSnipingCache) {
      ...
      //
      sniperMode.approve_gwei = Number(msg.text);
