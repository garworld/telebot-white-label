...
const { AUTOBUY_AMOUNT_OPTIONS_ID, AUTOBUY_SETTINGS } = require("../../constants/autobuy");
...
module.exports = async ({ bot, redis, msg }) => {
  ...
  //
  const autoBuyOptions = await redis.GET(msg.chat.id + AUTOBUY_AMOUNT_OPTIONS_ID);
  const autoBuyHandler = await redis.GET(msg.chat.id + "_autobuyamountmsg");

  autoBuyOptions ? await redis.DEL(msg.chat.id + AUTOBUY_AMOUNT_OPTIONS_ID) : null;
  autoBuyHandler ? await redis.DEL(msg.chat.id + "_autobuyamountmsg") : null;
  ...
  if (isNaN(Number(msg.text))) {
    ...
    bot.editMessageText("Wrong Amount Input", {
      parse_mode: "HTML",
    ...
    if (theAutoBuy) {
      ...
      if (msg.text[0] === ".") {
        autoBuyProperties.amount = Number("0" + msg.text);
      } else {
        autoBuyProperties.amount = Number(msg.text);
      }
