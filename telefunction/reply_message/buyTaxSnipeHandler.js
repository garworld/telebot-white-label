...
const { SNIPE_SETTINGS, SNIPE_BUY_TAX_OPTIONS_ID } = require("../../constants/sniping");
const { ethUsd } = require("../../helpers/tokenPrice");

module.exports = async ({ bot, redis, msg, chains }) => {
  //
  const snipeTokenOptions = await redis.GET(msg.chat.id + SNIPE_BUY_TAX_OPTIONS_ID);
  const snipeTokenHandler = await redis.GET(msg.chat.id + "_snipebuytaxmsg");

  snipeTokenOptions ? await redis.DEL(msg.chat.id + SNIPE_BUY_TAX_OPTIONS_ID) : null;
  snipeTokenHandler ? await redis.DEL(msg.chat.id + "_snipebuytaxmsg") : null;
  ...
  if (isNaN(Number(msg.text))) {
    ...
    bot.editMessageText("Wrong Tax on Buy Input", {
      parse_mode: "HTML",
    ...
    if (theSnipingCache) {
      ...
      //
      sniperMode.buy_tax = Number(msg.text);
