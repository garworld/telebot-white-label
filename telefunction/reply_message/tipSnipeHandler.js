...
const { SNIPE_SETTINGS, SNIPE_TIP_OPTIONS_ID } = require("../../constants/sniping");
const { ethUsd } = require("../../helpers/tokenPrice");

module.exports = async ({ bot, redis, msg, chains }) => {
  //
  const snipeTokenOptions = await redis.GET(msg.chat.id + SNIPE_TIP_OPTIONS_ID);
  const snipeTokenHandler = await redis.GET(msg.chat.id + "_snipetipmsg");

  snipeTokenOptions ? await redis.DEL(msg.chat.id + SNIPE_TIP_OPTIONS_ID) : null;
  snipeTokenHandler ? await redis.DEL(msg.chat.id + "_snipetipmsg") : null;

  if (isNaN(Number(msg.text)) || Number(msg.text) < 0.01) {
    ...
    bot.editMessageText("Wrong Tip Input", {
      parse_mode: "HTML",
    ...
    if (theSnipingCache) {
      ...
      //
      targetSnipe[0].tip = Number(msg.text);
