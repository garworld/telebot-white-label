...
const { SNIPE_SETTINGS, SNIPE_TOKEN_OPTIONS_ID } = require("../../constants/sniping");
const { ethUsd } = require("../../helpers/tokenPrice");

module.exports = async ({ bot, redis, msg, chains }) => {
  //
  const snipeTokenOptions = await redis.GET(msg.chat.id + SNIPE_TOKEN_OPTIONS_ID);
  const snipeTokenHandler = await redis.GET(msg.chat.id + "_snipetokenmsg");

  snipeTokenOptions ? await redis.DEL(msg.chat.id + SNIPE_TOKEN_OPTIONS_ID) : null;
  snipeTokenHandler ? await redis.DEL(msg.chat.id + "_snipetokenmsg") : null;
  ...
  if (theSnipingCache) {
    ...
    //
    targetSnipe[0].address = msg.text;
