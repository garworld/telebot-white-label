...
const { SNIPE_SETTINGS, SNIPE_AMOUNT_OPTIONS_ID } = require("../../constants/sniping");
const { ethUsd } = require("../../helpers/tokenPrice");

module.exports = async ({ bot, redis, msg, chains }) => {
  //
  const snipeTokenOptions = await redis.GET(msg.chat.id + SNIPE_AMOUNT_OPTIONS_ID);
  const snipeTokenHandler = await redis.GET(msg.chat.id + "_snipeamountmsg");

  snipeTokenOptions ? await redis.DEL(msg.chat.id + SNIPE_AMOUNT_OPTIONS_ID) : null;
  snipeTokenHandler ? await redis.DEL(msg.chat.id + "_snipeamountmsg") : null;
  ...
  if (isNaN(Number(msg.text))) {
    ...
    if (theSnipingCache) {
      ...
      //
      targetSnipe[0].amount = Number(msg.text);
