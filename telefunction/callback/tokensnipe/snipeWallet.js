...
module.exports = async ({ bot, msg, redis, action, chains }) => {
  //
  ...
  if (theSnipingCache) {
    ...
    switch (Number(action.split(":")[1])) {
      case 1:
        if (sniperMode.wallet_used.includes(wallet_number.FIRST)) {
          sniperMode.wallet_used.splice(
            sniperMode.wallet_used.indexOf(wallet_number.FIRST),
            1
          );
        } else {
          sniperMode.wallet_used.push(wallet_number.FIRST);
        }
        break;
      case 2:
        if (sniperMode.wallet_used.includes(wallet_number.SECOND)) {
          sniperMode.wallet_used.splice(
            sniperMode.wallet_used.indexOf(wallet_number.SECOND),
            1
          );
        } else {
          sniperMode.wallet_used.push(wallet_number.SECOND);
        }
        break;
      case 3:
        if (sniperMode.wallet_used.includes(wallet_number.THIRD)) {
          sniperMode.wallet_used.splice(
            sniperMode.wallet_used.indexOf(wallet_number.THIRD),
            1
          );
        } else {
          sniperMode.wallet_used.push(wallet_number.THIRD);
        }
        break;
      default:
        if (sniperMode.wallet_used.includes(wallet_number.FIRST)) {
